          ...
          {sessions.map((session) => (
            ...
            >
              ...
              <CardContent className="space-y-3">
                ...
                {/* Usage — driven by org subscription, not the legacy per-session daily cap */}
                <div className="space-y-1">
                  <div className="flex justify-between text-xs text-gray-500">
                    ...
                    <span>
                      {session.messages_sent_today} sent
                      {subscription && (
                        subscription.is_unlimited
                          ? " · Unlimited plan"
                          : ` · ${subscription.messages_remaining.toLocaleString()} left in plan`
                      )}
                    </span>
                  </div>
                  {/* Progress bar reflects the org's plan usage so all session cards stay in sync */}
                  {subscription && !subscription.is_unlimited && subscription.message_limit > 0 && (
